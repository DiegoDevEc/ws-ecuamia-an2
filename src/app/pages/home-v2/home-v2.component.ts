...
import { InformationFedexComponent } from '../popus/information-fedex/information-fedex.component';
...
export class HomeV2Component implements OnInit {
  ...
  carritoIncompletoSeleccionado = false

  private subscription: Subscription;

  constructor(public appService: AppService, private sharedService: SharedService, private datePipe: DatePipe,
    public appWebshopService: AppWebshopService, public dialog: MatDialog, private informarmationService: InformationService,
    private el: ElementRef, public router: Router, public snackBar: MatSnackBar, public responsive: ResponsiveService) {

    this.isMobile = this.responsive.isMobile();

    this.subscription = this.appService.eventoEditaCajaMixta.subscribe(() => {
    ...
    this.sharedService.dataProductUpdate.subscribe(async (data: any) => {

      this.productosPaginados = []

      let cajaMixtaArmada = this.appWebshopService.cajaMixtaArmada
      ...
      while (avanzar) {
        ...
        cajaMixtaArmada = this.appWebshopService.cajaMixtaArmada;
        let cajaArmada = this.appWebshopService.getCartLocalStorage();

        cajaMixtaArmada.detalle.length <= 0 ? avanzar = true : avanzar = false

        if (cajaArmada.cartListCaja.length > 0) {
          avanzar = false;
        }
    ...
    this.sharedService.carritoActual.subscribe(mensaje => {
      this.ngOnInit();
    });
  ...
  ngOnDestroy(): void {
    console.log("home-v2 component destroyed");
      ...
      if (data.length > 0) {
        ...
        dialogRef.afterClosed().subscribe(res => {
          ...
          if (res || res == undefined) {
            ...
            this.carritoIncompletoSeleccionado = res
            this.ngOnInit();
            return;
          }
        });
  ...
  async ngOnInit() {
    ...
    const paginador = this.appWebshopService.getPaginadorLocalStorage();
    ...
    if (selectContinueBuying == 'N') {
      this.obtenerCarritos();
    }

    if (!this.carritoIncompletoSeleccionado) {
      if (this.appWebshopService.paginador.aplicaFedex) {
  ...
  agregarACaja() {
    ...
    if (this.appWebshopService.paginador.aplicaFedex && !this.informarmationService.aplicaDestinatarioPedidoCompleto) {

      const dialogReFedex = this.dialog.open(InformationFedexComponent, {
        data: { data: 'N', marcacion: null, camion: null, pagina: 'HUB', tipo: null, destino: null },
        panelClass: 'information',
        disableClose: true,
        //maxHeight: '95vh'informat
      });
      dialogReFedex.afterClosed().subscribe(respuesta => {
  ...
  _tropicalFlowers() {
    ...
    const dialogRef = this.dialog.open(DeleteComponent, {
      data: {
        titulo: '',
        mensaje: '',
        mensajeDos: 'To place orders with tropical flowers, would you like to complete your existing order or start over?',
        imagen: 'C',
        starbuttons: 'S'
      },
      panelClass: 'delete-boxes'
  ...
  _iniciarPaginador() {
    if (this.appWebshopService.paginador.aplicaFedexCarrito) {
      this.appWebshopService.paginador.aplicaFedex = true
    } else {
      this.appWebshopService.paginador.aplicaFedex = this.usuario.aplicaFedex
    }
  ...
  getFechaPedido() {
    ...
    //return dateDateFormat.toString() 
    return dateDateFormat ? dateDateFormat.toString() : 'Fecha inválida';
  }

  _informationDialogControl(event) {
    this.appService.actualizarAumentarCajaMixta(event)

    this._informationDialog(this.marcacionSleccionada, this.appWebshopService.paginador.aplicaFedex ? 'T' : '')
  }
  ...
  public _informationDialog(marcacion, tipo) {
    ...
    tipo = this.appWebshopService.paginador.aplicaFedex ? 'T' : ''
    if (this.appWebshopService.data.cartListCaja.length !== 0 && !this.appWebshopService.paginador.aplicaFedex) {
      return
    }
  ...
  _limpiarWebShop() {
    this.carritoIncompletoSeleccionado = false
    this.appService.codigosProveedorFinales = [];
    this.appService.codigosProveedorRespaldo = [];
